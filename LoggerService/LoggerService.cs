...
public static class LoggerService
{
    ...
    private static List<LogEntry> logs_filtered = [];

    private static event Func<Task>? LogNotifier;

    public static bool autoExport = false;
    public static int autoExportSize = 1000;

    // Adds a message to the log list.
    public static void Log(string message, string? source = null, string? channel = null, string? level = null, string? data = null)
    {
    ...
    public static void Subscribe(Func<Task> listener)
    {
        LogNotifier += listener;
    }

    public static void Unsubscribe(Func<Task> listener)
    {
        LogNotifier -= listener;
    }
    ...
    public static List<LogEntry> GetLogs_Filtered()
    {
        return logs_filtered;
    }

    public static void filterLogs(string? search_message, string? search_source, string? search_timeStamp, string? search_channel, string? search_level, string? search_data)
    {
        logs_filtered.Clear();
    ...
    public static void ExportLogs(int totalToExport)
    {
        string dateTime = DateTime.Now.ToString().Replace("\\", "_").Replace(":", "-").Replace("/", "_");
        using (StreamWriter writer = new StreamWriter($"logs {dateTime}.csv"))
        {
    ...
    public static void ExportLogs(int? startingIndex, int? endingIndex) // FIXME: Needs to deal with null values
    {
    ...
    public static void ClearActiveLogs()
    {
        logs.Clear();
        LogNotifier?.Invoke();
    }

    public static void ClearActiveLogs(int? startingIndex, int? endingIndex)
    {
        int finalIndex = endingIndex ?? logs.Count - 1;
        int initialIndex = startingIndex ?? 0;
